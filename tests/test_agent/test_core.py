...
from ygents.agent.exceptions import AgentError, AgentConnectionError
...
async def test_problem_solved_detection(mock_agent_config):
    ...
    # Test positive cases
    assert agent._is_problem_solved({"type": "content", "content": "タスクは完了しました"})
    assert agent._is_problem_solved({"type": "content", "content": "問題は解決されました"})
    assert agent._is_problem_solved({"type": "content", "content": "Task is finished"})
    
    # Test negative cases
    assert not agent._is_problem_solved({"type": "content", "content": "まだ作業中です"})
...
async def test_abort_event(mock_agent_config, mock_litellm_streaming):
    ...
    async with Agent(mock_agent_config) as agent:
        ...
        # Since the mock response contains "完了しました", the problem might be solved normally
...
async def test_get_model_name_claude():
    ...
    from ygents.config.models import YgentsConfig, LLMConfig, ClaudeConfig
